...
from methods import vacancy_create, main_text, vacancy_to_text
...
async def confirm_vacancy(message: Message,
    ...
    await message.answer(text=vacancy_to_text(data, type_descr="short"),
...
async def callback_more_vacancy(callback: CallbackQuery,
                                state: FSMContext):
    data = await state.get_data()
    await callback.message.edit_text(text=vacancy_to_text(data, type_descr="long"),
...
async def callback_less_vacancy(callback: CallbackQuery,
                                state: FSMContext):
    data = await state.get_data()
    await callback.message.edit_text(text=vacancy_to_text(data, type_descr="short"),
                                     reply_markup=inkb_contact_like_more,
                                     parse_mode="MarkdownV2")
